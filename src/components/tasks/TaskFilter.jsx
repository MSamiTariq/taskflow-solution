...
const TaskFilter = () => {
  ...
  // Debounce search input to reduce re-renders and expensive filtering
  useEffect(() => {
  ...
  // Re-apply filters when tasks, status, or debounced search term change
  useEffect(() => {
    const activeFilters = { ...filters, search: debouncedSearch };
    applyFilters(tasks, activeFilters);
  }, [tasks, filters.status, debouncedSearch, applyFilters]);
      ...
      {filteredTasks.length === 0 ? (
        ...
        <ul className="divide-y divide-gray-200">
          {filteredTasks.map((task, index) => (
            <li
              key={task._id || `${task.title}-${task.dueDate || ""}-${index}`}
              className="py-4"
            >
